...
public class FileController {
	...
	public FileController(String catalog) {
		...
		//System.out.println(Arrays.toString(filesCollection.values().toArray()));
	}

	public static void main(String... args) {
		System.out.println(Arrays.toString(args));

		boolean globalPath = false;
		...
		for (int parameter = 0; parameter < args.length; parameter++) {
			switch (args[parameter]) {
				case "-g":
                    globalPath = true; // zmiana sposobu podawania ścieżki
                case "-l":
					if (isChoosePath) {
						throw new RuntimeException("Path was choose before " + (globalPath ? "-l" : "-g" + path));
					} else {
						isChoosePath = true;
		...
		if (path != null) {
			if (globalPath)
                ...
                controller = new FileController(System.getProperty("user.dir") + path);
